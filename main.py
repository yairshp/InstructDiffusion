...
class DataModuleFromConfig:
    ...
    def _train_concat_dataloader(self):
        ...
        #     self.datasets["ds1"], Txt2ImgIterableBaseDataset
        # )
            ...
            if config.trainer.accumulate_grad_batches > 1:
                ...
                if config.trainer.clip_grad > 0.0:
                    ...
                    grad_norm = torch.nn.utils.clip_grad_norm_(
                        model.parameters(), config.trainer.clip_grad
                    )
                else:
...
if __name__ == "__main__":
    ...
    logger = create_logger(
        output_dir=logdir, dist_rank=dist.get_rank(), name=f"{nowname}"
    )
    ...
    model_ema = LitEma(
        model, decay_resume=config.model.params.get("ema_resume", 0.9999)
    )
    ...
    if config.model.params.deepspeed != "":
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[opt.local_rank], broadcast_buffers=False
        )
